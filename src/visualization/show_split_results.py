        ...
        for n_sample in n_samples:
            ...
            for method in methods:
                df_method = pd.read_csv(
                    f"results/split/{dataset}/{n_sample}_samples_{method}_{split}.tsv",
                    sep="\t",
                )
            ...
            for i, metric in enumerate(metrics):
                ...
                if metric in ["test_spearman", "test_pearson"]:
                    ax_i.set_ylim([0, 1])

            ax[0, 1].set_axis_off()

            plt.suptitle(
                f"Split results ({split}): {dataset} ({n_sample} samples)", size=16
            )
            plt.tight_layout()
            if save_fig:
                plt.savefig(
                    f"figures/split_results_{n_sample}_samples_{dataset}_{split}.png"
                )
            plt.show()
