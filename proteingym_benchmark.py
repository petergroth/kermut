...
from kermut.data import filter_datasets, prepare_GP_inputs, prepare_GP_kwargs, split_inputs, standardize
...
def main(cfg: DictConfig) -> None:
    ...
    for i, (DMS_id, target_seq) in enumerate(df_ref.itertuples(index=False)):
        ...
        for test_fold in unique_folds:
            ...
            y_train, y_test = standardize(y_train, y_test) if cfg.data.standardize else (y_train, y_test)
            ...
            gp, likelihood = instantiate_gp(cfg=cfg, train_inputs=train_inputs, y_train=y_train, gp_inputs=gp_inputs)
        ...
        out_path = Path(cfg.data.paths.output_folder) / cfg.split / cfg.kernel.name / f"{DMS_id}.csv"
